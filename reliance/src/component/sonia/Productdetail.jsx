    ...
    <>
      ...
      <Stack w='100%' direction='flex' gap={20} mt={10}>
        <Box
          ...
          margin='auto'
          ml='10'
          border={"10px dashed #1a237e"}
          borderRadius='27px'
        >
          <Image src={data.url} w='100%' alt='' borderRadius='15px' />
        </Box>
        <Box w={"100%"} p={3}>
          <Heading fontSize={22} mb={10}>
            {data.name}
          </Heading>
          <Stack w='100%' direction={"flex"} gap='20px'>
            {" "}
            <Stack
              w={"100%"}
            ...
            >
              <Box fontSize={13}>
                <Heading fontSize={17} mb={3}>
                  Gain more with offers (2)
                </Heading>
                <ul style={{ marginLeft: "15px" }}>
                  <li>
                    7.5% Instant Discount (Max 5000) with Citibank Credit/Debit
                    Cards & EMI transactions. Read T&C
                  </li>
                  <li>
              ...
              <Box fontSize={13} mt={8}>
                ...
                <ul style={{ marginLeft: "15px" }}>
                  <li>
                    Processor AMD Ryzen 3 3250 RAM- 8 GB Internal Storage- 256
                    GB Storage Type- SSD
                  </li>
                </ul>
              </Box>
            ...
            >
              <Box>
                ...
                <p fontSize={17}>
                  Deal Price:
                  <b style={{ color: "blue", fontSize: "30px" }}>
                    {" "}
                ...
                <p fontSize={17}>
                  ...
                  >
                    {" "}
                    ₹{(data.price + (data.price * 5) / 100).toLocaleString()}
                  </b>{" "}
                  (Inclusive of all taxes)
                </p>
                <p fontSize={17} style={{ color: "16px" }}>
                  You Save :
                  {(data.discount ? data.discount : 5).toLocaleString()}%
                  <b style={{ fontSize: "16px" }}>
                    {" "}
                    (₹
                    {(
                      (data.price * (data.discount ? data.discount : 5)) /
                      100
                    ).toLocaleString()}
                    )
                  </b>
                ...
                <p>
                  ...
                  {(data.price * 3) / 100 +
                    ((data.price * 4) / 100).toLocaleString()}
                  /month
                </p>
                <Heading fontSize={17} mt={5}>
                  FREE Shipping!
                </Heading>
              </Box>
              <Box>
                ...
                {input && input.length === 6 ? (
                  <>
                    {" "}
                    <Box display='flex' fontSize={12.5}>
                      {" "}
                      <Image
                      ...
                      />
                      <p>
                        Standard Delivery: ( Within Next three working days)
                      </p>
                    </Box>
